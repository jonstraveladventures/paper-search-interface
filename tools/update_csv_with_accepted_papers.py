#!/usr/bin/env python3
"""
Update the CSV file with paper counts from our AI research analysis (ACCEPTED PAPERS ONLY).
...
def load_paper_counts():
    """Load the paper counts from our filtered analysis"""
    try:
        # Load the CSV we generated earlier
        df = pd.read_csv('global_country_papers_accepted_only_2015_2024.csv')
        ...
        print(f"📊 Loaded paper counts for {len(paper_counts)} countries (ACCEPTED PAPERS ONLY)")
        return paper_counts
        
    except FileNotFoundError:
        print("❌ Error: global_country_papers_accepted_only_2015_2024.csv not found. Please run extract_global_country_data_accepted_only.py first.")
        return None
...
def update_csv_with_papers(input_file, output_file, paper_counts):
    ...
    header[3] = 'Number of papers (accepted only)'
...
def main():
    """Main function"""
    print("🔄 Updating CSV with accepted paper counts only...")
    ...
    output_file = '/Users/jonathanshock/Downloads/data_with_accepted_papers.csv'
